package com.xionce.doctorvetServices;

import android.content.Intent;
import android.os.Bundle;
import android.view.View;

import androidx.annotation.Nullable;

import com.google.android.material.textfield.TextInputLayout;
import com.xionce.doctorvetServices.data.Get_pagination;
import com.xionce.doctorvetServices.data.Symptom;
import com.xionce.doctorvetServices.data.SymptomsAdapter;
import com.xionce.doctorvetServices.utilities.HelperClass;
import com.xionce.doctorvetServices.utilities.MySqlGson;

import java.util.ArrayList;

public class SearchSymptomActivity extends SearchActivityBase implements HelperClass.AdapterOnClickHandler {

    private static final String TAG = "SearchSymptomActivity";
    private SymptomsAdapter symptomsAdapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        TextInputLayout txtBuscar = findViewById(R.id.txt_search);
        txtBuscar.setHint(getString(R.string.symptom_search));
        search(true);
        setRecyclerView();

        Intent intent = new Intent(SearchSymptomActivity.this, EditSDTActivity.class);
        intent.putExtra(DoctorVetApp.REQUEST_CREATE_OBJECT, DoctorVetApp.INTENT_VALUES.SYMPTOM_OBJ.name());
        setCreateListener(intent);
    }

    @Override
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        super.onActivityResult(requestCode, resultCode, data);

        if (resultCode != RESULT_OK) return;

        if (requestCode == HelperClass.REQUEST_CREATE && data != null) {
            Intent dataBackIntent = getIntent();
            dataBackIntent.putExtra(DoctorVetApp.INTENT_VALUES.SYMPTOM_OBJ.name(), data.getStringExtra(DoctorVetApp.INTENT_VALUES.SYMPTOM_OBJ.name()));
            dataBackIntent.putExtra("GET_MODE", "CREATED");
            this.setResult(RESULT_OK, dataBackIntent);
            finish();
        }
    }

    @Override
    public void search(boolean on_first_fill) {
        if (!on_first_fill && !validateEmptySearch())
            return;

        resetPage();
        showProgressBar();
        hideFailSearch();
        DoctorVetApp.get().getSymptomsPagination(getSearchText(), getPage(), new DoctorVetApp.VolleyCallbackPagination() {
            @Override
            public void onSuccess(Get_pagination pagination) {
                hideProgressBar();
                //hideStartSearch();
                if (pagination != null) {
                    showBottomBar();
                    setTotal_pages(pagination.getTotal_pages());

                    ArrayList<Symptom> symptoms = ((Symptom.Get_pagination_symptoms)pagination).getContent();
                    symptomsAdapter = new SymptomsAdapter(symptoms, DoctorVetApp.AdapterSelectTypes.NORMAL);
                    symptomsAdapter.setOnClickHandler(SearchSymptomActivity.this);
                    recyclerView.setAdapter(symptomsAdapter);

//                    if (symptoms.size() == 0)
//                        setSugiereUnoListener(EditSDT_suggestedActivity.class);

                    manageShowRecyclerView(symptomsAdapter, on_first_fill);

                    showSoftKeyboard();
                } else {
                    DoctorVetApp.get().handle_null_adapter("symptoms", TAG, true);
                    showErrorMessage();
                }
            }
        });
    }

    public void doPagination() {
        isLoading = true;
        showProgressBar();
        addPage();
        DoctorVetApp.get().getSymptomsPagination(getSearchText(), getPage(), new DoctorVetApp.VolleyCallbackPagination() {
            @Override
            public void onSuccess(Get_pagination pagination) {
                hideProgressBar();
                if (pagination != null) {
                    ArrayList<Symptom> symptoms = ((Symptom.Get_pagination_symptoms)pagination).getContent();
                    symptomsAdapter.addItems(symptoms);
                    isLoading = false;
                } else {
                    DoctorVetApp.get().handle_null_adapter("symptoms", TAG, true);
                    showErrorMessage();
                }
            }
        });
    }

    @Override
    public void onClick(Object data, View view, int pos) {
        Symptom symptom = (Symptom) data;
        Intent dataBackIntent = getIntent();
        dataBackIntent.putExtra(DoctorVetApp.INTENT_VALUES.SYMPTOM_OBJ.name(), MySqlGson.getGson().toJson(symptom));
        dataBackIntent.putExtra("GET_MODE", "SEARCHED");
        this.setResult(RESULT_OK, dataBackIntent);
        finish();
    }

}
